...
function login($usernames, $passs, $ingat)
{
	...
	$auth = mysqli_query($koneksi, "SELECT * FROM user WHERE username='$username' and password='$pass' ");
...
function register($niks, $namas, $usrnms, $passs, $telps)
{
	...
	if(!preg_match('/[0-9]{1,17}$/', $nik)){
		...
		$cekduplikatuser = mysqli_num_rows(mysqli_query($koneksi, "SELECT * FROM user WHERE username='$usrnm'"));
		$cekduplikatuserpro = mysqli_num_rows(mysqli_query($koneksi, "SELECT * FROM petugas WHERE username='$usrnm'"));
		$cekduplikatnik = mysqli_num_rows(mysqli_query($koneksi, "SELECT * FROM user WHERE nik='$nik'"));
		if($cekduplikatuser > 0){
			...
			$register = mysqli_query($koneksi, "INSERT INTO user (nik, nama, username, password, telp) VALUES ('$nik', '$nama', '$usrnm', '$pass', '$telp')") or die ("<h1>ILEGAL TEXT DETECTED !</h1><b>TERJADI KESALAHAN PADA SISTEM HARAP HUBUNGI ADMINISTRATOR</b>");
...
function ajukan($nik, $telp, $path, $filename, $isis, $type, $opd)
{
	global $koneksi;
	global $tgl;
	...
	if($spam > 0){
		...
		$adukan = mysqli_query($koneksi, "INSERT INTO pengaduan (tgl_pengaduan, nik, telp, type, opd, isi_laporan, foto, status) VALUES ('$tgl', '$nik', '$telp', '$type','$opd','$isi','$filename', '1')") or die ("<h1>ILEGAL TEXT DETECTED !</h1><b>TERJADI KESALAHAN PADA SISTEM HARAP HUBUNGI ADMINISTRATOR</b>");
...
{	
	global $koneksi;
	$upd = mysqli_fetch_array(mysqli_query($koneksi, "SELECT * FROM user WHERE nik='$nik' "));
...
{
	global $koneksi;
	$cek = mysqli_query($koneksi, "SELECT * FROM user WHERE nik='$nik' and password='$pass' ");
	$auth = mysqli_num_rows($cek);
	if($auth > 0){
		mysqli_query($koneksi, "UPDATE user set nama='$nama', telp='$telp' WHERE nik='$nik'");
...
{
	global $koneksi;
	$cek = mysqli_query($koneksi, "SELECT * FROM user WHERE nik='$nik' and password='$pass_now' ");
	$auth = mysqli_num_rows($cek);
	if($auth > 0){
		if($pw1==$pw2){
			mysqli_query($koneksi, "UPDATE user set password='$pw2' WHERE nik='$nik'");
			header("Location:?page=pass&berhasil=passoke");
		}else{
			header("Location:?page=pass&berhasil=konf");	
		}
